...
void tcp_echoclient_connect(u8 tcp_offset)
{
    ...
    if (tcp_client_list[tcp_offset].echoclient_pcb != NULL)
    {
        ...
        //		netif = ip_route(&(tcp_client_list[tcp_offset].echoclient_pcb->remote_ip));
        //		if( tcp_client_list[tcp_offset].echoclient_pcb->local_ip.addr != netif->ip_addr.addr)
        //		{
        //		    tcp_client_list[tcp_offset].echoclient_pcb->local_port = 0;
        //                    tcp_bind(tcp_client_list[tcp_offset].echoclient_pcb,&(netif->ip_addr),tcp_client_list[tcp_offset].echoclient_pcb->local_port);
        //		}
    ...
    else if(es->state == ES_CONNECTED)
    {
        for(i = 0; i < TCP_CLIENT_MAX_NUM; i++)
        {
            if( tpcb->remote_port == tcp_client_list[i].dest_port )
            {
                ...
                if( tcp_client_list[i].tcp_client_statue == CLIENT_CONNECT_OK)
                {
...
void  tcp_client_process(void)
{
    ...
    else if( tcp_client_list[tcp_offset].echoclient_pcb->state == ESTABLISHED)
    {
        ...
        if( heart_dely == 0 )
        {
            send_heart_msg(&(tcp_client_list[tcp_offset].tcp_send_buf[0]), &(tcp_client_list[tcp_offset].tcp_send_len), HEART_MSG_TYPE); 
            tcp_write(tcp_client_list[tcp_offset].echoclient_pcb, &(tcp_client_list[tcp_offset].tcp_send_buf[0]), tcp_client_list[tcp_offset].tcp_send_len, 1);
            heart_dely = 30;
        }
    }
    //tcp_client_statue = tcp_client_list[tcp_offset].tcp_client_statue;
}
